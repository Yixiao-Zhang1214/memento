...
3. 回答或跳过后生成默认润色正文；
4. 继续补充内容，或主动让馆员再问一个不重复的问题；
5. 每轮都用完整历史重新生成正文，可按意愿持续对话；
6. 保留当前版本，或调整预设及自定义风格；
7. 用户最终确认收藏后，再展示短来源和一句馆员评语。
...
测试使用模拟上游响应，覆盖模型选择、认证头、超时重试、图片证据、逐轮问题预算、
历史问题去重、多轮内容覆盖、默认成稿、风格调整、Skill 契约和前端密钥隔离。
